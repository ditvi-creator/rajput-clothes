...
export default function SplitHero() {
  return (
    <section className="relative h-[50vh] sm:h-screen w-full flex flex-row overflow-hidden">
      {/* MEN SIDE */}
      <Link to="/shop?category=Men" className="flex-1">
        <motion.div 
          whileHover={{ flex: 1.2 }}
        ...
        >
          ...
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-4 sm:p-8">
            <motion.h2 
              ...
              className="text-3xl sm:text-7xl font-serif text-white mb-4 italic text-center"
            >
              Men
            </motion.h2>
            <div className="h-px w-0 group-hover:w-24 bg-brand-gold transition-all duration-500" />
            <p className="mt-4 text-[8px] sm:text-xs tracking-[0.3em] uppercase text-white opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500 text-center">
              Heritage Bold
            </p>
          </div>
        </motion.div>
      ...
      {/* WOMEN SIDE */}
      <Link to="/shop?category=Women" className="flex-1">
        <motion.div 
          whileHover={{ flex: 1.2 }}
          transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
          className="group relative h-full overflow-hidden cursor-pointer"
