...
export default function SplitHero() {
  return (
    <section className="relative h-[90vh] sm:h-screen w-full flex flex-col sm:flex-row overflow-hidden">
      ...
      >
        <div className="absolute inset-0 bg-brand-onyx/40 z-10 group-hover:bg-brand-onyx/20 transition-colors duration-500" />
        <img 
        ...
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-8">
          <motion.h2 
            ...
            className="text-5xl sm:text-7xl font-serif text-brand-white mb-4 italic"
          >
            Men
          </motion.h2>
          <div className="h-px w-0 group-hover:w-24 bg-brand-gold transition-all duration-500" />
          <p className="mt-4 text-xs tracking-[0.3em] uppercase text-brand-white opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">
            Heritage Bold
          </p>
        </div>
      </motion.div>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-30 hidden sm:block">
        <div className="bg-brand-onyx px-8 py-3 border border-brand-gold/30">
           <p className="text-[10px] tracking-[0.8em] text-brand-gold uppercase">EST. 1996</p>
        </div>
      </div>
    </section>
  );
